...
import com.varun.ClaimProcessingApplication.dto.UserCredentialsDTO;
import com.varun.ClaimProcessingApplication.dto.UserDTO;
import com.varun.ClaimProcessingApplication.entity.User;
import com.varun.ClaimProcessingApplication.service.UserService;
...
public class UsersController {
     ...
     * @param request HttpServletRequest providing request information for logging
     *                or headers.
     * @return ResponseEntity with the registered user and HTTP status 201
     *         (Created).
     */
    ...
    public ResponseEntity<?> loginUser(@RequestBody UserCredentialsDTO credentials, HttpServletRequest request) {
        ...
        if (user != null) {
            ...
            String token = JWT.create()
                    ...
                    .withExpiresAt(new Date(System.currentTimeMillis() + 10 * 60 * 1000)) // Set expiration time to 10
                                                                                          // minutes from now
                    .withIssuer(request.getRequestURL().toString()) // Set the 'issuer' claim to the URL where the
                                                                    // request was made
                    .sign(Algorithm.HMAC256("secret")); // Sign the token with HMAC256 algorithm using the 'secret' key

            // Return the ResponseEntity with status OK, and add the token in the
            // Authorization header
            return ResponseEntity.ok().header(HttpHeaders.AUTHORIZATION, "Bearer " + token).body(user);
        } else {
            // If the user is not found or the authentication failed, return an unauthorized
            // status
     ...
     * @param updatedUser User entity with updated user details from the request
     *                    body.
